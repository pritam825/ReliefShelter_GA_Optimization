...
def nearestShelter(Shelterset,locn):
        ...
        for i in range(P):
               distances[i] = geodesic((AreaLatitudes[Shelterset[i]], AreaLongitudes[Shelterset[i]]), (AreaLatitudes[locn], AreaLongitudes[locn])).miles
...
def calcPopScore(chromosome,ShelterList):
        ...
        for i in range(P):
                for j in range(len(ShelterList[i])):
                        PopScore += AreaPopulations[ShelterList[i][j]] / (1.0000 + geodesic((AreaLatitudes[chromosome[i]], AreaLongitudes[chromosome[i]]), (AreaLatitudes[ShelterList[i][j]], AreaLongitudes[ShelterList[i][j]])).miles)
        return PopScore
...
def calcMaxAvgDist(chromosome,ShelterList):
        ...
        for i in range(P):
                sumDist=0
                for j in range(len(ShelterList[i])):
                       sumDist += geodesic((AreaLatitudes[chromosome[i]], AreaLongitudes[chromosome[i]]), (AreaLatitudes[ShelterList[i][j]], AreaLongitudes[ShelterList[i][j]])).miles
